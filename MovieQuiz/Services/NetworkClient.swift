...
extension NetworkClient {
    func handleNetworkError(_ error: Error) -> NetworkErrors {
        // Проверка типа ошибки и возвращение соответствующего значения NetworkErrors
        if let urlError = error as? URLError, urlError.code == .notConnectedToInternet {
            return .loadImageError("Интернет-соединение отсутствует")
        } else {
            return .loadImageError("Неизвестная ошибка сети")
        }
    }
}
